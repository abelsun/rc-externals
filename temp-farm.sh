...
#for i in 0;
do
  script=scripts/script_${targ}_${e0[i]}_${th[i]}.xml
  cp  temp-farm_template.xml ${script}
  sed -i "s|%%TARGET%%|${targ}|g" -- ${script}
  sed -i "s|%%ANGLE%%|${th[i]}|g" -- ${script}
  sed -i "s|%%E0%%|${e0[i]}|g" -- ${script}
  jsub -xml ${script}
done
